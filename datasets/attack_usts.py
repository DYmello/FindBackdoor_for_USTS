...
class PoisonWorker:
    ...
    def set_backdoor(self, atk_cls, tar_cls, backdoor, cpos, size, attack):
        ...
        if not os.path.exists(self.im_bd):
            os.mkdir(self.im_bd)
        if self.bdname != 'ysq':
            self.backdoor = cv2.imread('./For_USTS_DATABASE/BadNets/datasets/%s_nobg.png' % self.bdname, -1)
...
if __name__ == '__main__':
    ...
    images_dict = pickle.load(open('./usts/pickles/images_dict.pkl', 'rb'))
    clean_set_trn = pickle.load(open('./usts/pickles/clean_set_trn.pkl', 'rb'))
    clean_set_tst = pickle.load(open('./usts/pickles/clean_set_tst.pkl', 'rb'))
    ...
    for prefix, backdoor in enumerate(backdoors, prefix_start):
        ...
        attacked_set = set(p.map(poison, enumerate(images_dict.items(), 0)))
        print_flush('Done.')
